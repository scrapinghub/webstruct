...
class CRFsuiteTest(unittest.TestCase):
    ...
    def _get_Xy(self, num, enc=IobEncoder()):
        trees = get_trees(num)
        html_tokenizer = webstruct.HtmlTokenizer(tagset=self.TAGSET, sequence_encoder=enc)
        return html_tokenizer.tokenize(trees)

    def _get_train_test(self, train_size, test_size, enc=IobEncoder()):
        X, y = self._get_Xy(train_size+test_size, enc)
    ...
    def test_training_tagging_Bilou(self):
        X_train, X_test, y_train, y_test = self._get_train_test(8, 2, BilouEncoder())
